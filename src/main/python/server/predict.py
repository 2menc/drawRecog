...
def load_categories(model_name):
    ...
    if not os.path.exists(labels_path):
        print(f"WARNING: labels file not found ({labels_path}). Using fallbacks categories.")
        CATEGORIES = [f"Class_{i}" for i in range(1000)] # Fallback fittizio
        return

    with open(labels_path, 'r', encoding='utf-8') as f:
        CATEGORIES = [line.strip() for line in f if line.strip()]
    print(f"loaded {len(CATEGORIES)} categories for {model_name}.")
...
def change_model(model_name):
    ...
    try:
        ...
        print("model and categories loaded")
    except Exception as e:
        print(f"error in model {model_name} loading: {e}")
...
def predict(input_data):
    ...
    if predicted_idx < len(CATEGORIES):
        ...
        print(f"WARNING: the model has predicted the index {predicted_idx}, but there are only {len(CATEGORIES)} categories in the list")
